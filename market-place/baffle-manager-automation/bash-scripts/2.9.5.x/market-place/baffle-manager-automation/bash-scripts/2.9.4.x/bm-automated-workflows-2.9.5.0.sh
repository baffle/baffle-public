...
create_rqe_db_install_udfs(){
 ...
 # Create database and tables
  ...
  echo "Creating UDFs..."
  execution_status=$(execute_sql_file "$db_host_name" "$db_port" "$db_user_name" "$db_name" "/home/ec2-user/AE-2/rqe_base_plpgsql_pg_tle.sql")
  execution_status=$(execute_sql_file "$db_host_name" "$db_port" "$db_user_name" "$db_name" "/home/ec2-user/AE-2/rqe_udfs_plpgsql_pg_tle.sql")
  if [ "$execution_status" == "error" ]; then
    echo "Database and table creation failed. Exiting script." >&2
    exit 1
  fi
